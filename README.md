...
**2. a) Initializations (Quick start script)**

Run the setup script to get a quick start on installing XPRESSO.
...
- [ WARNING ]: in linux servers, make sure the max*map_count is set to at \_least* 262144, ie `vm.max_map_count=262144`. See [elastic documentation](https://www.elastic.co/guide/en/elasticsearch/reference/current/vm-max-map-count.html) for details.
...
- Ensure the `ADVERTISED_URL` in the `.env` file is set to the fully qualified external URL using hostname or IP _not localhost_
